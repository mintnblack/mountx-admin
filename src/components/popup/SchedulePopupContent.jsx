...
function SchedulePopupContent(props) {
  ...
  const onScheduleAppointment = () => {
    if (onHandleValidation()) {
      ...
      axios
        .put(`${BASE_URL}/appointment/pending/schedule/${appointmentId}`, data)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${BASE_URL}/timeslot/?clinic=${appointmentInfo.clinic_id}&date=${appointmentInfo.date}`
      )
      .then((response) => {
  ...
  return (
    <div className={Design.SchedulePopup}>
      ...
      <div className={Design.scheduledAppointment}>
        <p>Clinic Name</p>
        <h3>{appointmentInfo.clinic}</h3>
        <p>Appointment Date</p>
        <h3>{appointmentInfo.date}</h3>
        <p>Select Timeslot</p>
